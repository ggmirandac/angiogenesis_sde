...
function simulate_sprout(
    ...
    normal_dist = Distributions.Normal(0, Δτ)
    normal_rand = rand(normal_dist, (2, n_steps))
...
xa = [0., 0.]
δ = 1.8 # > 0  implies there is more chemotactic drift towards the source, < 0 more random
Δτ = .01
n_steps = Int(10000)
...
# final_y, final_x = [], []
x_lists, y_lists = zeros(10, n_steps), zeros(10, n_steps)
for i = 1:10
    x_plot, v_plot = simulate_sprout(
                    x1, # initial position and velocity
                    δ, # adimentioanl chemotactic responsiveness
...
function autocorr_vect(x_vect)
    autocorr_v = Any[]
    for i = 1:size(x_vect, 1)
       push!(autocorr_v, autocor(x_vect[i, :]))
    end
    return autocorr_v
end

autocorr_x = autocorr_vect(x_lists)
autocorr_y = autocorr_vect(y_lists)
Plots.plot(autocorr_x, label = "")
Plots.plot!(autocorr_y, label = "")
