...
begin
    ...
    Pkg.instantiate()
	using Plots, Distributions, Brownian, DataFrames, LaTeXStrings, Measures, StatsBase
end
...
begin
	...
	for i = 1:n_s
			xv = simulate_sprout(x1, nabla_a, beta, alpha, kappa, xa, ya; T = 100, N = 300, R = 2, v1 = [0;0], a0 = 2, Radious = Radious)
		x = xv[1]
...
# ╔═╡ 7aceec32-699b-407f-820d-137d782d280e
begin
	n_reps = 1e4
	x_list = Any[]
	y_list = Any[]
	...
	for i = 1:n_reps
		...
		x_vect = xv[1]
		push!(x_list, x_vect[:,"x"])
		push!(y_list, x_vect[:, "y"])

		v_vect = xv[2]
		push!(vx_list, v_vect[:,"x"])
		push!(vy_list, v_vect[:,"y"])
...
# ╔═╡ ec78e5de-804a-4526-88cd-70c7846f7563
Plots.plot(StatsBase.autocor(vy_list[10]; demean = true))
